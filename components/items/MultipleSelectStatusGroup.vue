...
export default {
  ...
  computed: {
    valueArr () {
      return this.$store.getters.getSelectedStatusGroupFilter
    },
    valid () {
      return this.selectedStatusGroup !== null
    },
    optionsStatusGroup () {
      return this.$store.getters.getOptionsStatus
    }
  },
  mounted () {
    this.updateValueObg()
  ...
  methods: {
    updateValueObg () {
      const obg = {}
      const arr = JSON.parse(JSON.stringify(this.optionsStatusGroup))
    ...
    switchItem (item) {
      ...
      this.$store.commit('setSelectedStatusGroupFilter', newArr)
    }
  }
}
</script>
